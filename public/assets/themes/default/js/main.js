...
const logoDarkImage = document.querySelector('#logo-dark-image');
...
(function () {
  ...
  menuToggler.forEach(item => {
    item.addEventListener('click', event => {
      ...
      // تحديث الصور بناءً على حالة القائمة
      updateImages();
    });
  });
  ...
  /**
   * دالة لتحديث الشعار والفيفيكون بناءً على حالة القائمة والوضع الحالي
   */
  function updateImages() {
    ...
    // 🧠 تحقق من حجم الشاشة
    const isMobile = window.innerWidth <= 768; // أو استخدم 576 لو تحب


    if (isCollapsed && !isMobile) {
      // حالة القائمة مطوية: إظهار الفيفيكون وإخفاء الشعارين
      if (faviconImage) {
      ...
      if (logoDarkImage) {
        logoDarkImage.style.display = 'none';
      }
    } else {
      // حالة القائمة مفتوحة: إخفاء الفيفيكون وإظهار الشعار المناسب بناءً على الوضع
      if (faviconImage) {
      ...
      if (currentTheme === 'light') {
        ...
        if (logoDarkImage) {
          logoDarkImage.style.display = 'block';
        }
      } else {
        if (logoImage) {
  ...
  if (layoutMenu) {
    layoutMenu.addEventListener('mouseenter', function () {
      ...
      if (isCollapsed && !isMobile) {
        // إخفاء الفيفيكون
        if (faviconImage) faviconImage.style.display = 'none';

        // إظهار اللوجو المناسب حسب الثيم
        const currentTheme = window.Helpers.getPreferredTheme();
        if (currentTheme === 'light') {
          if (logoImage) logoImage.style.display = 'none';
          if (logoDarkImage) logoDarkImage.style.display = 'block';
        } else {
          if (logoImage) logoImage.style.display = 'block';
          if (logoDarkImage) logoDarkImage.style.display = 'none';
        }
      }
    });
    ...
    layoutMenu.addEventListener('mouseleave', function () {
      ...
      if (isCollapsed && !isMobile) {
        // إظهار الفيفيكون
        if (faviconImage) faviconImage.style.display = 'block';
  ...
  // تحديث الصور عند تغيير الوضع
  const originalSetTheme = window.Helpers.setTheme;
  window.Helpers.setTheme = function (theme) {
    originalSetTheme.call(this, theme);
    updateImages();
  };

  // تحديث الصور عند تغيير حجم النافذة إذا لزم الأمر
  window.addEventListener('resize', function (event) {
    updateImages();
  }, true);

  // تحديث الصور عند تعديل أي حالة أخرى تؤثر على الشعار أو الفيفيكون
  // مثل swipeIn، swipeOut، أو أي دوال أخرى تتحكم في الحالة
