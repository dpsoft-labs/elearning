...
(function () {
  ...
  // دالة لتحديث اللوجو بناءً على الثيم الحالي
  function updateLogo(theme) {
    const logoImage = document.querySelector('#logo-image');
    const logoDarkImage = document.querySelector('#logo-dark-image');

    if (logoImage && logoDarkImage) {
      if (theme === 'light') {
        // عند الثيم الفاتح نعرض اللوجو الداكن ونخفي الفاتح
        logoImage.style.display = 'none';
        logoDarkImage.style.display = 'block';
      } else {
        // عند الثيم الداكن نعرض اللوجو الفاتح ونخفي الداكن
  ...
  // تحديث اللوجو بناءً على الثيم الحالي
  updateLogo(storedStyle);
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-bs-theme-value]').forEach(toggle => {
      toggle.addEventListener('click', () => {
        ...
        // تحديث اللوجو عند تغيير الثيم
        updateLogo(currTheme);
      });
    });
  });
})();
