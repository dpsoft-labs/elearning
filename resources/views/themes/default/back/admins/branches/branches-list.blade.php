            ...
            @can('add branches')
                <div class="card-action-element mb-2" style="text-align: end;">
                    <button type="button" class="btn btn-primary waves-effect waves-light" data-bs-toggle="modal"
                        ...
                        <i class="fa fa-plus fa-xs me-1"></i> {{ __('l.Add New Branch') }}
                    </button>
                </div>
                    ...
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content p-3 p-md-5">
                            <div class="modal-header">
                                <h3 class="modal-title text-center">{{ __('l.Add New Branch') }}</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form id="addBranchForm" class="row g-3" method="post"
                                    action="{{ route('dashboard.admins.branches-store') }}">
                                    @csrf
                    ...
                    <table class="table" id="branches-table">
                        ...
                        <tbody>
                            @foreach($branches as $index => $branch)
                                <tr>
                                    <td>
                                        <input type="checkbox" class="form-check-input row-checkbox" value="{{ $branch->id }}">
                                    </td>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $branch->name }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            @can('show branches')
                                                <a href="{{ route('dashboard.admins.branches-show', ['id' => encrypt($branch->id)]) }}"
                                            ...
                                            @can('delete branches')
                                                <button type="button" class="btn btn-sm btn-danger delete-branch"
                                                        data-id="{{ encrypt($branch->id) }}" title="{{ __('l.Delete') }}">
        ...
        $(function() {
            ...
            $('#deleteSelected').on('click', function() {
                ...
                if (selectedIds.length > 0) {
                    Swal.fire({
                        ...
                        text: "{{ __('l.Selected branches will be deleted!') }}",
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            window.location.href =
                                "{{ route('dashboard.admins.branches-deleteSelected') }}?ids=" +
                                selectedIds.join(',');
                        }
                    });
                }
            ...
            $(document).on('click', '.delete-branch', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href =
                            "{{ route('dashboard.admins.branches-delete') }}?id=" +
                            branchId;
                    }
                });
            });
