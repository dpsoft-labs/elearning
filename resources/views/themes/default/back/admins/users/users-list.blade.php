            ...
            @if ($inactiveUsers == 0)
                @can('add users')
                    <div class="card-action-element mb-2" style="text-align: end; ">
                        <a href="{{ route('dashboard.admins.users-import-get') }}" class="btn btn-info waves-effect waves-light me-2">
                            <i class="fa fa-file-import ti-xs me-1"></i>
                            @lang('l.Import Users')
                        </a>
                        <a href="{{ route('dashboard.admins.users-add') }}" class="btn btn-primary waves-effect waves-light"><i
                ...
                @can('delete users')
                    <div class="card-action-element mb-2" style="text-align: end; ">
                        <a href="javascript:void(0);" class="btn btn-danger waves-effect waves-light delete-all-inactive"><i
                                class="fa fa-trash ti-xs me-1"></i>@lang('l.Delete All Inactive Users')
                        </a>
                    </div>
                @endcan
            @endif
                ...
                <ul class="nav nav-tabs nav-fill w-100" role="tablist" style="min-height: 50px;">
                    <li class="nav-item">
                        <a class="nav-link {{ $inactiveUsers == 0 ? 'active' : '' }}"
                            href="{{ route('dashboard.admins.users') }}" style="padding: 15px;">
                            <i class="fa fa-users ti-xs me-1"></i> @lang('l.Active Users')
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ $inactiveUsers == 1 ? 'active' : '' }}"
                            href="{{ route('dashboard.admins.users') }}?inactive=1" style="padding: 15px;">
                            <i class="fa fa-user-times ti-xs me-1"></i> @lang('l.Inactive Users')
                        </a>
                    </li>
                </ul>
            </div>
        ...
        $(function() {
                ...
                buttons: [{
                        ...
                        buttons: [{
                                ...
                                action: function(e, dt, node, config) {
                                    e.preventDefault();
                                    window.open(
                                        '{{ route('dashboard.admins.users.export') }}?type=excel&' +
                                        $.param({
                                            search: {
                                                value: table.search()
                                            },
                            ...
                            },
                            {
                                ...
                                action: function(e, dt, node, config) {
                                    e.preventDefault();
                                    window.open(
                                        '{{ route('dashboard.admins.users.export') }}?type=csv&' +
                                        $.param({
                                            search: {
                                                value: table.search()
                                            },
            ...
            function exportData(type) {
                if (type === 'print' || type === 'pdf') {
                    $.ajax({
                        url: '{{ route('dashboard.admins.users.export') }}',
                        type: 'GET',
                        data: {
                            search: {
                                value: table.search()
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href =
                            '{{ route('dashboard.admins.users-delete-allinactive') }}';
                    }
                });
            });
            ...
            $('#deleteSelected').on('click', function() {
                ...
                if (selectedIds.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href =
                                '{{ route('dashboard.admins.users-delete-selected') }}?ids=' +
                                selectedIds.join(',');
                        }
                    });
                }
