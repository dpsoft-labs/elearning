                ...
                <div class="row g-0">
                    ...
                    <div class="col app-chat-sidebar-left app-sidebar overflow-hidden" id="app-chat-sidebar-left">
                        <div
                            ...
                            <div class="avatar avatar-xl avatar-online chat-sidebar-avatar">
                                <img src="{{ asset(auth()->user()->photo) }}" alt="Avatar" class="rounded-circle">
                            </div>
                            <h5 class="mt-4 mb-0">{{ auth()->user()->firstname . ' ' . auth()->user()->lastname }}</h5>
                        ...
                        <div class="sidebar-body ps ps--active-y">
                            ...
                            <ul class="list-unstyled chat-contact-list py-2 mb-0" id="chat-list">
                                <li class="chat-contact-list-item chat-contact-list-item-title mt-0">
                                    <h5 class="text-primary mb-0">@lang('l.Previous User Chats')</h5>
                                </li>
                                @forelse($ticket->user->tickets()->orderByDesc('id')->get() as $tic)
                                    <li class="chat-contact-list-item mb-1 @if ($ticket->id == $tic->id) active @endif">
                                        <a href="{{ route('dashboard.admins.tickets-show', ['id' => encrypt($tic->id)]) }}"
                                            class="d-flex align-items-center text-secondary">
                                            <div
                                                class="flex-shrink-0 avatar @if ($tic->status == 'answered') avatar-info @elseif($tic->status == 'in_progress') avatar-warning blink @else avatar-offline @endif">
                                    ...
                                    <div class="d-flex overflow-hidden align-items-center">
                                        ...
                                        <div class="chat-contact-info flex-grow-1 ms-4">
                                            <h6 class="m-0 fw-normal">
                                                {{ $ticket->user->firstname . ' ' . $ticket->user->lastname }}</h6>
                                            <small class="user-status text-body">{{ $ticket->user->email }}</small>
                                        </div>
                                    </div>
                                                ...
                                                @if ($ticket->status != 'closed')
                                                    @can('edit tickets')
                                                        <a class="dropdown-item"
                                                            href="{{ route('dashboard.admins.tickets-close') }}?id={{ encrypt($ticket->id) }}">
                                                            <i class="fa fa-window-close me-md-1 me-0"></i> @lang('l.Close Ticket')
                                                        </a>
                                                    @endcan
                                                @else
                                                    @can('edit tickets')
                                                        <a class="dropdown-item"
                                                            href="{{ route('dashboard.admins.tickets-active') }}?id={{ encrypt($ticket->id) }}">
                                                            <i class="fa fa-bolt me-md-1 me-0"></i> @lang('l.Active Ticket')
                                                        </a>
                                                    @endcan
                                                @endif
                                                @can('delete tickets')
                                                    <a class="dropdown-item deleteTicket" href="javascript:void(0);"
                                                        data-delete-id="{{ encrypt($ticket->id) }}">
                                                        <i class="fa fa-trash me-md-1 me-0"></i> @lang('l.Delete Ticket')
                                                    </a>
                                                @endcan
                                            </div>
                                        </div>
                            ...
                            @if ($ticket->status != 'closed')
                                <div class="chat-history-footer shadow-xs">
                                    <form class="d-flex justify-content-between align-items-center"
                                        method="post" action="{{ route('dashboard.admins.tickets-reply') }}"
                                        ...
                                        <textarea name="message" class="form-control message-input border-0 me-4 shadow-none" autofocus
                                            placeholder="@lang('l.Type your message here...')" id="messageInput" required></textarea>
                    ...
                    <div class="col app-chat-sidebar-right app-sidebar overflow-hidden" id="app-chat-sidebar-right">
                        <div
                            ...
                            <div
                                class="avatar avatar-xl @if ($ticket->user->isOnline()) avatar-online @else avatar-offline @endif chat-sidebar-avatar">
                                <img src="{{ asset($ticket->user->photo) }}" alt="Avatar" class="rounded-circle">
                            </div>
                            <h5 class="mt-4 mb-0">{{ $ticket->user->firstname . ' ' . $ticket->user->lastname }}</h5>
                        ...
                        <div class="sidebar-body p-6 pt-0 ps ps--active-y">
                            <div class="my-6">
                                <p class="text-uppercase mb-1 text-muted">@lang('l.Client Information')</p>
                                <ul class="list-unstyled d-grid gap-4 mb-0 ms-2 py-2 text-heading">
                                    ...
                                    <li class="d-flex align-items-center">
                                        ...
                                        <span class="align-middle ms-2">{{ $ticket->user->phone }}</span>
                                    </li>
                                    <li class="d-flex align-items-center">
                                        <i class="bx bx-time-five"></i>
                                        <span
                                            class="align-middle ms-2">{{ $ticket->user->created_at->format('d/m/Y') }}</span>
                                    </li>
                                </ul>
                            </div>
                            <div class="d-flex mt-6">
                                <a href="{{ route('dashboard.admins.users-show') }}?id={{ encrypt($ticket->user_id) }}"
                                    class="btn btn-info w-100">
                                    @lang('l.Show Client') <i class="bx bx-show bx-sm ms-2"></i>
                                </a>
                            </div>
                            <div class="ps__rail-x" style="left: 0px; bottom: 0px;">
                                <div class="ps__thumb-x" tabindex="0" style="left: 0px; width: 0px;"></div>
...
@section('js')
        ...
        $(document).on('click', '.deleteTicket', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '{{ route('dashboard.admins.tickets-delete') }}?id=' +
                        inactiveId;
                }
            });
        });
    ...
    <script>
        ...
        function checkNewMessages() {
            $.ajax({
                url: '{{ route("dashboard.admins.tickets-get-new-messages") }}',
                method: 'GET',
                data: {
