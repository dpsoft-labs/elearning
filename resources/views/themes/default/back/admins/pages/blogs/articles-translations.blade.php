    ...
    @can('edit blog')
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="card-action-element mb-2" style="text-align: end;">
                <a href="{{ route('dashboard.admins.blogs.articles-auto-translate', ['id' => encrypt($blog->id)]) }}"
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">@lang('l.Translate Article')</h3>
                </div>
                <div class="card-body">
                    <form id="translateForm" class="row g-3" method="post"
                        action="{{ route('dashboard.admins.blogs.articles-translate') }}">
                        @csrf @method('patch')
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <input type="text" name="meta_keywords-{{ $language->code }}"
                                            ...
                                            value="{{ $blog->getTranslation('meta_keywords', $language->code, false) ?? '' }}"
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea name="meta_description-{{ $language->code }}"
                                            ...
                                            maxlength="160" rows="3">{{ $blog->getTranslation('meta_description', $language->code, false) ?? '' }}</textarea>
                                    </div>

                                    <div class="col-12 mb-2">
                                        <label class="form-label">@lang('l.Title') ({{ $language->native }})</label>
                                        <input type="text" name="title-{{ $language->code }}"
                                            class="form-control translation-input"
                                            value="{{ $blog->getTranslation('title', $language->code) ?? '' }}"
                                            placeholder="@lang('l.Enter title in') {{ $language->native }}" required />
                                    </div>
                                    <div class="col-12 mb-2">
                                        <label class="form-label">@lang('l.Content') ({{ $language->native }})</label>
                                        <div id="content-{{ $language->code }}" style="height: 300px; overflow-y: auto;">
                                            {!! $blog->getTranslation('content', $language->code) ?? '' !!}
                                        </div>
                                        <input type="hidden" name="content-{{ $language->code }}"
                                            id="content-input-{{ $language->code }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
...
@section('js')
    <script>
        ...
        @foreach ($languages as $language)
            editors['{{ $language->code }}'] = new Quill('#content-{{ $language->code }}', {
                theme: 'snow',
                modules: {
                    toolbar: fullToolbar
                },
            ...
            document.getElementById('translateForm').addEventListener('submit', function() {
                document.getElementById('content-input-{{ $language->code }}').value =
                    editors['{{ $language->code }}'].root.innerHTML;
            });
        @endforeach
