<div class="tab-pane fade show active" id="v-pills-General" role="tabpanel" aria-labelledby="v-pills-General-tab">
    <div class="card shadow-sm">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h5 class="mb-0">
                <i class="fas fa-shield-alt me-2"></i>@lang('l.Security Settings')
            </h5>
        </div>
            ...
            <form action="{{ route('dashboard.admins.settings-update') }}" method="POST">
                ...
                <div class="row mt-5">
                    <div class="col-md-4 mb-3">
                        <label for="max_sessions" class="form-label">@lang('l.Max Sessions') <i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" title="@lang('l.The maximum number of active sessions allowed for each user (number of login attempts at the same time)')"></i></label>
                        <input type="number" class="form-control" id="max_sessions" name="max_sessions"
                            value="{{ old('max_sessions', $settings['max_sessions']) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="session_timeout" class="form-label">@lang('l.Session Timeout (hours)') <i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" title="@lang('l.The time period in hours before the user session expires automatically in the event of inactivity')"></i></label>
                        <input type="number" class="form-control" id="session_timeout" name="session_timeout"
                            value="{{ old('session_timeout', $settings['session_timeout']) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="max_attempts" class="form-label">@lang('l.Max Login Attempts') <i class="fas fa-info-circle ms-1" data-bs-toggle="tooltip" title="@lang('l.The maximum number of failed login attempts before temporarily locking the account')"></i></label>
                    ...
                    <div class="col-md-12 mb-3 mt-4">
                        ...
                        <div class="d-flex align-items-center gap-4">
                            ...
                            <div>
                                <img src="{{ asset('assets/themes/default/img/icons/recaptcha.gif') }}" alt="Recaptcha Demo" class="img-fluid" style="max-width: 200px;">
                            </div>
                        </div>
                    </div>
                    <div class="alert alert-info alert-dismissible fade show" role="alert">
                        @lang('l.If you need help getting these credentials, please read our detailed guide at') <a href="{{env('recaptcha_guide_url')}}" target="_blank">@lang('l.Here')</a>
                ...
                <button type="submit" class="btn btn-primary">@lang('l.Save Changes')</button>
            </form>
        </div>
    </div>
    ...
    <div class="card mt-4 shadow-sm">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h5 class="mb-0">
                <i class="fas fa-ban me-2"></i>@lang('l.Blocked IPs')
            </h5>
            <button class="btn btn-info" data-bs-toggle="modal"
                data-bs-target="#addIpModal">@lang('l.Add IP')</button>
        </div>
        <div class="card-body p-4">
                    ...
                    <tbody>
                        @forelse ($ips as $ip)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-danger btn-sm delete-ip" data-id="{{ encrypt($ip->id) }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center">@lang('l.No data found')</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addIpModalLabel">@lang('l.Add New IP')</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('dashboard.admins.firewalls-store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="ip" class="form-label">@lang('l.IP Address')</label>
                            <input type="text" class="form-control" id="ip" name="ip" required>
                        </div>
                        <button type="submit" class="btn btn-primary">@lang('l.Add IP')</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    button.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `{{ route('dashboard.admins.firewalls-delete') }}?id=${ipId}`;
            }
        });
    });
});
