    ...
    <div class="card shadow-sm">
        <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center py-5 mb-5 ">
            <h5 class="mb-0 text-white">
                <i class="fas fa-download me-2"></i>@lang('l.Updates & Backups')
            </h5>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-4">
                            <button class="btn btn-info" id="createBackupBtn">
                                <i class="fas fa-plus me-2"></i>@lang('l.Create New Backup')
                            </button>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>@lang('l.Filename')</th>
                                        <th>@lang('l.Size') (MB)</th>
                                ...
                                <tbody>
                                    @forelse($backups as $backup)
                                        <tr>
                                            ...
                                            <td>{{ $backup['date'] }}</td>
                                            <td>
                                                <a href="{{ asset('backup/laravel/' . $backup['name']) }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center">@lang('l.No backups found')</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
...
<script>
    ...
    function createBackup() {
        Swal.fire({
            title: '@lang('l.Confirm Backup')',
            text: '@lang('l.Are you sure you want to create a new backup? This may take several minutes.')',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: '@lang('l.Yes, create it!')',
            cancelButtonText: '@lang('l.Cancel')'
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.fire({
                    title: '@lang('l.Creating Backup')',
                    ...
                    didOpen: () => {
                        Swal.showLoading();
                        setTimeout(function() {
                            window.location.href = "{{ route('dashboard.admins.backup-take') }}";
                        }, 500);
                    }
                });
            }
    ...
    function confirmDelete(backupName) {
        Swal.fire({
            ...
            text: '@lang("l.You won\'t be able to revert this!")',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                window.location.href = "{{ route('dashboard.admins.backup-delete') }}?backup=" + backupName;
            }
        });
    }
</script>
