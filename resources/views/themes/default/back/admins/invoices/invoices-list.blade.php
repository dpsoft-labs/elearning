                    ...
                    <tbody>
                        @foreach ($invoices as $invoice)
                            <tr>
                                <td>
                                    <a
                                        href="{{ route('dashboard.admins.invoices-show', ['invoice_id' => encrypt($invoice->id)]) }}">
                                        #{{ $invoice->id }}
                                    </a>
                                </td>
                                <td>
                                    <div class="d-flex justify-content-start align-items-center">
                                        ...
                                        <div class="d-flex flex-column">
                                            <a href="{{ route('dashboard.admins.students-show', ['id' => encrypt($invoice->user_id)]) }}"
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('dashboard.admins.invoices-show', ['invoice_id' => encrypt($invoice->id)]) }}"
                                            class="btn btn-sm btn-icon">
                                            <i class="bx bx-show mx-1"></i>
                                        </a>
                                        @can('delete invoices')
                                            <a href="javascript:void(0)" class="btn btn-sm btn-icon delete-invoice" data-invoice-id="{{ encrypt($invoice->id) }}">
    ...
    <script>
        ...
        $('.delete-invoice').on('click', function(e) {
            ...
            Swal.fire({
                ...
                text: "{{ __('l.This invoice will be deleted!') }}",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ route('dashboard.admins.invoices-delete', ['invoice_id' => ':invoice_id']) }}".replace(':invoice_id', invoice_id);
                }
            });
        });
    </script>
