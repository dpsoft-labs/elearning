...
@section('css')
    ...
    {{-- <link rel="stylesheet" href="{{ asset('assets/themes/default/css/ai.css') }}" type="text/css" /> --}}
    <style>
        :root {
            ...
            <div class="chat-main">
                <div class="chat-header">
                    <div class="chat-title">
                        <i class='bx bx-bot'></i> @lang('l.AI Assistant')
                    </div>
                </div>
                ...
                <div id="chat-messages">
                    <div class="empty-chat" id="empty-state">
                        ...
                        <h3>@lang('l.How can I help you today?')</h3>
                        <p>@lang('l.Ask me anything or describe what you need help with.')</p>
                    </div>
                </div>

                <div class="bottom">
                    <div class="input-container">
                        <textarea id="message-input" placeholder="@lang('l.Type your message here...')"></textarea>
                        <button class="send-button" id="send-button">
                            <i class='bx bx-send'></i>
                        </button>
                    </div>
                    <div class="stats-container">
                        <p class="note">* @lang('l.Use English language to reduce token usage')</p>
                        <p>@lang('l.Remaining tokens today'): <span id="remaining">{{ $remaining }}</span></p>
                    </div>
                </div>
            </div>
        </div>
...
@section('js')
    <script>
        setInterval(function() {
            ...
            xhr.open('GET', '{{ route('dashboard.users.ai-getremaining') }}', true);
            xhr.onload = function() {
                if (this.status == 200) {
    ...
    <script>
        ...
        function send_message() {
            ...
            fetch("{{ route('dashboard.users.ai-message') }}", {
                    method: "POST",
                    headers: {
                ...
                .catch(error => {
                    update_message(message, '@lang("l.Sorry, there was an error processing your request.")');
